"""
Geospatial analysis for congestion zone - MEMORY OPTIMIZED
"""
import pandas as pd
...
def calculate_compliance_rate(ddf):
    """
    Calculate surcharge compliance rate - MEMORY EFFICIENT
    
    Uses aggregation instead of loading full dataframes
    """
    print("   Filtering trips entering zone after Jan 5, 2025...")
    
    try:
        # FIX: Use .sum() on boolean mask instead of .shape[0]
        ...
        # Count using sum (memory efficient)
        ...
        # Count compliant trips (those with surcharge > 0)
        ...
        # Find top 3 locations with missing surcharges
        print("   Identifying top leakage locations...")
        
        # Filter to non-compliant trips only
        non_compliant_mask = combined_mask & (ddf['congestion_surcharge'] == 0)
        
        # Group and count (this aggregates before computing)
        leakage_by_location = (
...
def analyze_border_effect(ddf):
    """
    Analyze border dropoff patterns - MEMORY EFFICIENT
    """
    ...
    try:
        ...
        # Filter to Q1 border dropoffs only
        print("   Filtering Q1 border dropoffs...")
        q1_border = ddf[is_q1 & is_border]
        
        # Aggregate BEFORE computing (this is key for memory efficiency)
        ...
        if 2024 in border_comparison.columns and 2025 in border_comparison.columns:
            # Vectorized calculation (avoid loops)
            border_comparison['pct_change'] = (
                (border_comparison[2025] - border_comparison[2024]) / 
                border_comparison[2024].replace(0, 1)  # Avoid division by zero
            ) * 100
            
            # Set pct_change to 0 where 2024 was 0
